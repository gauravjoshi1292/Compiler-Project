           ...
           while(!isEmptyStack(st))
           {
                ...
                if(tmp.type==1)
                {
                       ...
                       if(tmp.terminal!=$)
                       {
							if(tmp.terminal==Eps)
							{
								//if sibling is null go to its parent whose sibling is not null 
								while(pNode->sibling==NULL)
								{
									if(pNode->s.nonterminal==program)
									{
								...
								//printf("hi\n");
								pNode=pNode->sibling;//otherwise goto the sibling
							}
							else
							{
								pNode->t = temp_token;//put tokeninfo in the node
								//printf("** %d ** %d\n",pNode->t.type,temp_token.type);

								// now go to its parent whose sibling is not null
								while(pNode->sibling==NULL)
								{
									if(pNode->s.nonterminal==program)
									{
                ...
                else if(tmp.type==0)
                {
                       ...
                       printf("xxxx %d  %d \t",tmp.nonterminal,temp_token.type);
